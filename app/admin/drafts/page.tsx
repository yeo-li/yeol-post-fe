...
import { Search, Eye, Edit, Trash2, Calendar, Clock, FileText, FolderOpen, Settings, Loader2, Lock, Save, Menu } from 'lucide-react'
import { deletePost, fetchLoginInformation, fetchAllDraftPosts } from "@/lib/api"
import Link from "next/link"
import { useRouter } from "next/navigation"

const sidebarItems = [
    { icon: FileText, label: "포스트 관리", active: false, href: "/admin" },
    { icon: Save, label: "임시저장 목록", active: true, href: "/admin/drafts" },
...
export default function AdminDraftsPage() {
    ...
    const checkLoginAndLoadData = async () => {
        ...
        try {
            ...
            // 2. 로그인되어 있으면 게시물 목록 로드 (임시저장만 필터링)
            const draftPosts = await fetchAllDraftPosts()
            setPosts(draftPosts)
        } catch (error) {
    ...
    const handleEditDraft = (postId: number) => {
    ...
    const handleDeleteConfirm = async () => {
        ...
        try {
            ...
            alert("임시저장 글 삭제 중 오류가 발생했습니다. 다시 시도해주세요.")
        } finally {
            setDeleting(false)
        }
    }
    ...
    return (
        <div className="min-h-screen bg-background">
                ...
                <div className="flex-1 p-4 md:p-6 lg:p-8">
                    ...
                    <div className="flex items-center justify-between mb-6 md:mb-8">
                        <div className="flex items-center gap-4">
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <h1 className="text-2xl md:text-3xl font-bold">임시저장 목록</h1>
                            </div>
                        </div>
                    </div>
                    <p className="text-sm md:text-base text-muted-foreground mb-6">
                        아직 출간하지 않은 임시저장된 글들을 관리할 수 있습니다. 총 {posts.length}개의 임시저장 글이 있습니다.
                    </p>
                    ...
                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-4 md:mb-6 gap-4">
                        <div className="relative flex-1 max-w-md">
                            ...
                            <Input
                                placeholder="임시저장 글 제목, 카테고리, 작성자, 태그로 검색..."
                                value={searchQuery}
                        ...
                        {filteredPosts.length === 0 ? (
                            <div className="text-center py-12">
                                ...
                                <h3 className="text-lg font-semibold mb-2">
                                    {searchQuery ? "검색 결과가 없습니다" : "임시저장된 글이 없습니다"}
                                </h3>
                                <p className="text-muted-foreground mb-4">
                                    {searchQuery ? "다른 검색어로 시도해보세요." : "새로운 글을 작성해보세요!"}
                                </p>
                                {!searchQuery && (
                                    <Link href="/admin/write">
                                        <Button className="gap-2 bg-foreground text-background hover:bg-foreground/90">
                                ...
                                >
                                    <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                                        <div className="flex-1">
                                            ...
                                            <h2 className="text-xl font-semibold mb-3 line-clamp-2 cursor-pointer hover:text-primary transition-colors"
                                                onClick={() => handleEditDraft(post.postId)}>
                                                {post.title || "제목 없음"}
                                            </h2>
                                        ...
                                        <div className="flex items-center gap-2 self-end md:self-start md:ml-4">
                                            <Button
                                                variant="ghost"
                                                size="icon"
                                                className="hover:bg-muted text-amber-600 hover:text-amber-700"
                                                onClick={() => handleEditDraft(post.postId)}
                                                title="계속 작성하기"
                                            >
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 md:gap-4 mt-8">
                        <Card>
                            <CardContent className="p-3 md:p-4 lg:p-6 text-center">
                                <div className="text-xl md:text-2xl lg:text-3xl font-bold mb-1 text-amber-600">{posts.length}</div>
                                <div className="text-xs md:text-sm text-muted-foreground">임시저장 글</div>
                            </CardContent>
                        </Card>
                        <Card>
            ...
            <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
                <DialogContent>
                    <DialogHeader>
                        <DialogTitle>임시저장 글 삭제</DialogTitle>
                        <DialogDescription>
                            정말로 이 임시저장 글을 삭제하시겠습니까? 이 작업은 되돌릴 수 없습니다.
                        </DialogDescription>
                    </DialogHeader>

                    {postToDelete && (
                        <div className="my-4 p-4 bg-muted/50 rounded-lg border-l-4 border-l-amber-500">
                            <h4 className="font-semibold mb-2">{postToDelete.title || "제목 없음"}</h4>
